...
export function Cloth() {
  ...
  useEffect(() => {
    ...
    function ParticleSystem(canvasId) {
      ...
      var onMouseMoveCallback = function (ev) {
        if (this.clickCon.active) {
          ...
          this.clickCon.y = 400 - ev.offsetY;
    ...
    var rows = 40;
    var cols = 40;
    var startX = 900 / 2.0 - (rows * cWidth) / 2.0;

    for (var i = 0; i < rows; i++) {
      for (var j = 0; j < cols; j++) {
        var index = i * rows + j;
        var positionX = startX + j * cWidth;
        var positionY = 400 - i * cHeight;
    ...
    const interval = setInterval(ps.step.bind(ps), 20); // Run the main loop.
    return () => {
      clearInterval(interval);
    }
  }, []);

  return <canvas id="screen" width={900} height={400} ref={canvas}></canvas>;
}
